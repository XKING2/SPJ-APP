document.addEventListener("DOMContentLoaded", () => {
    ...
    // Widget Toggle
    btnToggle.onclick = () => widget.style.display = "flex";
    ...
    window.openChat = function(userId) {
        ...
        if (!userId) {
            console.error("openChat() error: userId undefined");
            return;
        }
...
function makeDraggable(element) {
    ...
    // Aktifkan drag
    makeDraggable(document.getElementById("chat-widget"));        // seluruh widget geser
    makeDraggable(document.querySelector(".chat-header"));         // header list geser
    makeDraggable(document.querySelector(".chat-room-header"));    // header chat room geser
    makeDraggable(document.getElementById("chat-toggle-btn"));     // tombol bulat geser

    // =============================
// REALTIME LISTENER VIA ECHO
// =============================
if (window.Echo) {
    const userId = window.currentUserId; // pastikan ini di-set di blade <script> sebelum chat.js

    Echo.private(`chat.${userId}`)
        .listen('.MessageSent', (e) => {
            console.log("Realtime message:", e);

            // Jika chat room untuk sender sedang terbuka
            if (currentChatId == e.sender.id || currentChatId == e.receiver_id) {
                const messagesBox = document.getElementById("chat-messages");

                // Tentukan bubble kiri/kanan
